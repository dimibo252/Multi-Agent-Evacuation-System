        ...
        async def run(self):
            ...
            if txt:
                if txt.body.startswith("Fire") and self.agent.role == 'fireman':
                    ...
                    print(
                        f"{self.agent.role} responder is responding to fire in Room: {target_floor},{target_row},{target_col}.")
                    ...
                    await self.agent.navigate_to_room(target_room)
                    print(
                        f"{self.agent.role} responder is responding to security threat in Room: {target_floor},{target_row},{target_col}.")
                    print(
                        f"{self.agent.role} Security threat in Room: {target_floor},{target_row},{target_col} resolved.")
    ...
    async def send_resolution_status(self, emergency_type, room=None):
        ...
        if emergency_type in ["Fire", "Earthquake", "Security Threat"]:
            if room:
                ...
                print(
                    f"EmergencyResponder: Mensagem enviada ao BMS sobre {emergency_type} resolvido na sala ({room.row},{room.col},{room.floor}).")
        else:
        ...
        while self.location != target_room:
            if self.location.floor != target_room.floor:
                ...
                if not self.go_to_next_room(target_room):
                    print(
                        f"{self.role.capitalize()} responder: Couldn't reach Room {target_room.row},{target_room.col}.")
                    return
